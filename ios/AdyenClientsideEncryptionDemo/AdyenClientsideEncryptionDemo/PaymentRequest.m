...
- (void)submitWithMerchantAccount:(NSString *)merchantAccount
    ...
    Encrypter* encrypter = [[Encrypter alloc] initWithVersion:EncryptionParametersVersion1
                                                    publicKey:publicKey];
    NSError* error;
    NSString* encryptedCreditCardData = [encrypter encrypt:[self.card encode] error:&error];
